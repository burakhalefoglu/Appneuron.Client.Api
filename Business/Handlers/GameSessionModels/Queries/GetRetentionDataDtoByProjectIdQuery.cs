...
namespace Business.Handlers.GameSessionModels.Queries
{
    public class GetRetentionDataDtoByProjectIdQuery : IRequest<IDataResult<RetentionDataWithSessionDto>>
    {
        ...
        public class GetRetentionDataDtoByProjectIdQueryHandler : IRequestHandler<GetRetentionDataDtoByProjectIdQuery,
            IDataResult<RetentionDataWithSessionDto>>
        {
            private readonly IGameSessionModelRepository _gameSessionEveryLoginDataRepository;
            private readonly IMediator _mediator;

            public GetRetentionDataDtoByProjectIdQueryHandler(
            ...
            public async Task<IDataResult<RetentionDataWithSessionDto>> Handle(
                GetRetentionDataDtoByProjectIdQuery request, CancellationToken cancellationToken)
            {
                ...
                var minSessionDate =
                    DateTime.Parse(sessionData.ToList().Min(s => s.SessionFinishTime).ToString("dd, MM , yyyy"));
                var maxSessionDate =
                    DateTime.Parse(sessionData.ToList().Max(s => s.SessionFinishTime).ToString("dd, MM , yyyy"));
                var retentionDataList = new List<RetentionDataDto>();

                for (var date = minSessionDate; date <= maxSessionDate; date.AddDays(1))
                {
